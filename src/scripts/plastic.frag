...
struct MaterialProps {
  float metallic, roughness, occulusion;
};

const MaterialProps props = MaterialProps(1., 1., 1.);
const float uMetallic = 1.;
const float uRoughness = 1.;
const float uOcclusion = 1.;
...
void getIBLContribution(inout vec3 diffuse, inout vec3 specular, float NdV,
                        float roughness, vec3 n, vec3 reflection,
...
void main() {
  vec3 baseColor = SRGBtoLinear(texture(tBaseColor, vUv)).rgb;
  ...
  vec3 diffuseIBL;
  vec3 specularIBL;
  getIBLContribution(diffuseIBL, specularIBL, NdV, roughness, N, reflection,
