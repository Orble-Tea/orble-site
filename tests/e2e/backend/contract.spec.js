...
// Layer 3: the page talks to the REAL API routes running the REAL service
// code; only Nayax and Google Sheets are faked (fixture-server.mjs). These
// tests exist for one bug class only: contract drift between what
// restock-service.js actually returns and what the page expects. No UI
// behavior re-testing here; layer 2 owns that.
...
test("Load: real service output renders (field-name contract)", async ({
  ...
  const slot3 = page.locator("#slot-list > div").nth(2);
  ...
  const slot4 = page.locator("#slot-list > div").nth(3);
  ...
  await expect(page.locator("#slot-list > div").nth(4)).toContainText("Empty");
});
