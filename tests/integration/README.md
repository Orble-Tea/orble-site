...
These tests exercise the backend against real Google Sheets workbooks, using a service account, and the Nayax QA API.
...
- Live Nayax QA authentication and `machineProducts` reads
...
2. Fill in the test-only values below:
   ...
   - `NAYAX_BASE_URL=https://qa-lynx.nayax.com/operational/v1`
   - `NAYAX_QA_MACHINE_30TH_ID`
   - `NAYAX_QA_API_TOKEN`
...
The workflow at `.github/workflows/integration-tests.yml` runs the same suite on every push, using GitHub Secrets for the test workbook IDs, service account credentials, and Nayax QA credentials.
