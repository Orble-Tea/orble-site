...
// Layer 4: THE live smoke. Everything real: the deployed site, the real key,
// live Google Sheets, live Nayax. Its single job is proving the table the
// restocker sees is derived correctly from the Production Plan / Inventory
// and survives the trip through the real backend.
//
// INDEPENDENCE RULE: this file must NEVER import from src/lib/restock. The
// expectations are computed here, naively, from raw sheet values. If it
// reused the backend's own code, a derivation bug would produce the same
// wrong number on both sides of the assertion and pass.
//
// READ-ONLY RULE: this test never taps Complete. Once the real submit ships,
// that button writes actual Restock Log rows and Nayax updates.
//
// Run manually (see README "Live smoke test"):
//   RESTOCK_SECRET_KEY=... SMOKE_DATE=2026-09-08 npm run smoke
...
// invented end to end. Slot-level placement is layer 3's job (fixtures).
...
test("live derivation: rendered table corresponds to Production Plan / Inventory", async ({
  ...
  if (expectedEvent === "Load") {
    ...
    for (const row of plan) {
      ...
      const amount = Number(row[`Amount to ${MACHINE}`] || 0);
